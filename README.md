...
当测试对象是可视化前端 UI 时，`/product-toolkit:test-case` 产出必须满足以下强制项：

1. 使用 `agent-browser` 或 `browser-use` 执行真实浏览器测试。
2. 从登录页开始验证（登录成功后进入功能路径）。
3. 对关键步骤保留截图证据，并核验：
   - 数据是否正确绑定
   - 页面排版是否正常
4. 检查浏览器 Console：不得有未处理错误。
5. 检查 Network/API：关键接口响应状态必须为 HTTP 200。
6. 覆盖用户故事全部验收标准（AC）并输出 AC→TC 覆盖矩阵。
7. 测试账号、环境地址、角色权限信息仅可由用户提供；若账号信息不足以覆盖全部角色，结论必须标记为 `Blocked`。
8. 测试凭据不得以明文写入仓库文档，报告中仅记录角色与脱敏信息。

> 交付规则：以上任一项不满足，即判定为**不可交付**。
...
# 生成测试用例（完整覆盖，含版本化；UI 场景需通过可视化测试门槛）
